...
%% =========================================================
%  5.3) TORQUE-BASED SMC JOINT TRACKING
%  =========================================================
smc.lambda = [18, 18, 18];
smc.k      = [30, 30, 30];
smc.phi    = [0.12, 0.12, 0.12];

% Torque scaling
smc.Ka = [0.08, 0.12, 0.08];

% Equivalent inertia for MATLAB offline test
smc.J_eq = [1, 1, 1];

% Torque limit
smc.tau_max = [8, 12, 8];

smc_result = simulate_smc_torque_all_legs(t, joint_ref, smc);

plot_smc_torque_tracking_result(t, smc_result, 'FL');
...
function smc_result = simulate_smc_all_legs(t, joint_ref, smc)
    ...
    for k = 1:numel(legs)
        ...
        for i = 1:N-1
            ...
            tau_i = smc.Ka .* u_smc;
            tau_i = max(min(tau_i, smc.tau_max), -smc.tau_max);
...
function smc_result = simulate_smc_torque_all_legs(t, joint_ref, smc)
    % Mô phỏng SMC dạng torque xấp xỉ cho 4 chân.
    %
    % Giả sử động lực học đơn giản:
    % J_eq * qddot = tau
    %
    % Nếu chưa biết J_eq, tạm lấy J_eq = 1 cho mỗi khớp.
    % Khi sang Webots, q và qdot sẽ lấy trực tiếp từ robot thật trong mô phỏng.
    ...
    for k_leg = 1:numel(legs)
        ...
        % Quán tính tương đương xấp xỉ từng khớp
        if isfield(smc, 'J_eq')
            J_eq = smc.J_eq;
        else
            J_eq = [1, 1, 1];
        ...
        % Điều kiện đầu lệch để test bám
        q(1,:)     = qd(1,:) + [0.15, -0.10, 0.12];
        q_dot(1,:) = [0, 0, 0];
        ...
        for i = 1:N-1
            ...
            % Plant đơn giản để test offline:
            % J_eq * qddot = tau
            q_ddot_i = tau_i ./ J_eq;
        ...
        smc_result.(leg).tau = tau;
        smc_result.(leg).u_smc = u_smc;
        smc_result.(leg).s = s_log;
        smc_result.(leg).e = e_log;
...
function plot_smc_torque_tracking_result(t, smc_result, leg)
    q  = smc_result.(leg).q;
    qd = smc_result.(leg).qd;
    ...
    figure('Name',[leg ' Torque-SMC Joint Tracking'],'Color','w');
    ...
    grid on;
    ylabel('q [rad]');
    title([leg ' torque-SMC joint tracking']);
    legend('q1d','q1','q2d','q2','q3d','q3');
